...
def run_VIS_pipeline(input_dir, output_dir, mode='nodding', convert_ascii=False):
    ...
    VIS.SetOutputDir(str(output_dir))
    ...
    if convert_ascii:
        out1d = glob.glob(str(output_dir)+"/*FLUX_MERGE1D_VIS*.fits")
        fitsfile = fits.open(out1d[0])
        wave = 10.*(np.arange((np.shape(fitsfile[0].data)[0]))*fitsfile[0].header["CDELT1"]+fitsfile[0].header["CRVAL1"])
        np.savetxt(output_dir/"VIS_ASCII1D_spectrum.dat", list(zip(wave, fitsfile[0].data, fitsfile[1].data)), fmt="%1.4e %1.4e %1.4e")
